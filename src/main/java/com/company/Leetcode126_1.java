...
//        List<List<String>> ladders = leetcode126_1.findLadders("qa", "sq", wordList);
        List<List<String>> ladders = leetcode126_1.findLadders("sand", "acne", wordList);
    ...
    public List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList) {
        ...
        // posendWord 表示在edges中的终边为endWord的点
...
//            System.out.print(temp2);
            ...
            if (temp2.size() >  minInthePath(path)) {
                continue;

            }

            int nowPos = temp2.get(temp2.size() - 1);
//            System.out.println(" " + nowPos );
            if (nowPos == posendWord) {
//                System.out.println(temp2);
//                System.out.println(minInthePath(path) + " " + temp2.size());
                /*
                if (path.isEmpty()) {
                    path.add(temp2);
                    continue;
                ...
                }else if (temp2.size() == minInthePath(path)) {
//                    System.out.println(temp2);
                    path.add(temp2);
                } else {
                    path.removeAll(path);
...
//                    cost
//                }
                if (edges[nowPos][i] && !temp2.contains(i) ) {
                    if (cost[i] >= temp2.size() + 1) {
//                    if (cost[])
//                    if (true) {
                        cost[i] = temp2.size() + 1;
...
//        System.out.println(path);
        ...
        path){
            ...
            for (Integer p1 : p) {
                if (p1 == 0) a.add(beginWord);
                else {
                    a.add(wordList.get(p1 - 1));
                }
//                result.add(a);
            }
            result.add(a);
...
//        System.out.println(count);
    ...
    public int minInthePath (List<List<Integer>> path) {
//        int min = path.get(0).size();
        if (path.isEmpty()) return Integer.MAX_VALUE;
        int min = path.get(0).size();

        for (List<Integer> path1:
        path){
            if (path1.size() < min) min = path.size();
        }
